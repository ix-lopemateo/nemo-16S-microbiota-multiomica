#!/usr/bin/env Rscript
# Fase 1 clinica. Cribado univariante de variables clinicas
...
df <- read_excel("../data/nemo_wide_clean_con_leyenda.xlsx")
cat("Dataset cargado")
...
kable(candidate_summary,
      caption = "Resumen descriptivo de las 11 variables candidatas.") %>%
...
cor_data <- df %>%
  select(all_of(continuous_candidates)) %>%
  filter(complete.cases(.))

cat("Casos completos para la matriz de correlación: n =", nrow(cor_data), "\n")
...
write.csv(candidate_summary,
          "../R_output/tables/candidate_variable_summary_clinico.csv", row.names = FALSE)
write.csv(round(cor_mat, 3),
          "../R_output/tables/correlation_matrix_candidates_clinico.csv")
write.csv(univariate_cond,
          "../R_output/tables/univariate_screening_WLZ6_clinico.csv", row.names = FALSE)
write.csv(univariate_all,
          "../R_output/tables/univariate_screening_dWLZ06_clinico.csv", row.names = FALSE)

pdf("../R_output/figures/fig_corr_candidates_clinico.pdf", width = 10, height = 9)
corrplot(cor_mat, method = "color", type = "lower",
         tl.cex = 0.65, tl.col = "black",
         addCoef.col = "black", number.cex = 0.55,
         col = colorRampPalette(c("#2166AC", "white", "#B2182B"))(200),
...
# --- Objetos para el script 03_modelos_multivariante.Rmd ---
# Dataset analítico limpio
saveRDS(df, "../R_output/models/df_analytic_clinico.rds")
...
saveRDS(list(
  ...
  resumen         = resumen
), "../R_output/models/variable_selection_clinico.rds")

cat("Objetos guardados en output/models/ para el script 03.\n")
